...
class QQMusicContentProviderTest {
    ...
    fun factoryExposesQQMusicDiscoveryFeatures() = runTest {
        val graph = createFuoProviderGraph(InMemoryProviderCredentialStore())
        graph.registry.updateEnabledProviders(setOf("qqmusic"))

        val ids = graph.content.features().filter { it.providerId == "qqmusic" }.map { it.id }.toSet()
