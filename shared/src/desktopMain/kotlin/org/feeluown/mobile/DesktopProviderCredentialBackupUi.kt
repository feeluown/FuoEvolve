...
@Composable
internal fun DesktopProviderCredentialBackupDialogs(
    ...
    var importInspection by remember { mutableStateOf<DesktopProviderCredentialBackupInspection?>(null) }
    ...
    exportTarget?.let { target ->
        DesktopProviderCredentialExportDialog(
            backup = backup,
            target = target,
    ...
    pendingImport?.let { content ->
        importInspection?.let { inspection ->
            DesktopProviderCredentialImportDialog(
...
@Composable
private fun DesktopProviderCredentialExportDialog(
    backup: DesktopProviderCredentialBackup,
    target: DesktopProviderCredentialBackupTarget,
    onDismiss: () -> Unit,
    onExportFile: (String) -> Unit,
) {
    var mode by remember(target.providerId) { mutableStateOf(DesktopProviderCredentialExportMode.Encrypted) }
    ...
    val canExport = !isBusy && when (mode) {
        DesktopProviderCredentialExportMode.Encrypted -> encryptedReady
        DesktopProviderCredentialExportMode.Plaintext -> plaintextRiskAccepted
    }
    ...
    AlertDialog(
        ...
        text = {
            Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
                DesktopExportModeRow(
                    selected = mode == DesktopProviderCredentialExportMode.Encrypted,
                    title = "加密备份",
                    onClick = {
                        mode = DesktopProviderCredentialExportMode.Encrypted
                        plaintextRiskAccepted = false
                    },
                )
                DesktopExportModeRow(
                    selected = mode == DesktopProviderCredentialExportMode.Plaintext,
                    title = "明文 JSON",
                    onClick = { mode = DesktopProviderCredentialExportMode.Plaintext },
                )

                if (mode == DesktopProviderCredentialExportMode.Encrypted) {
                    OutlinedTextField(
                ...
                onClick = {
                    ...
                    scope.launch {
                        runCatching {
                            backup.export(
                                mode = mode,
                                password = if (mode == DesktopProviderCredentialExportMode.Encrypted) password else "",
...
@Composable
private fun DesktopExportModeRow(
    selected: Boolean,
    title: String,
    onClick: () -> Unit,
) {
...
@Composable
private fun DesktopProviderCredentialImportDialog(
    inspection: DesktopProviderCredentialBackupInspection,
    onDismiss: () -> Unit,
