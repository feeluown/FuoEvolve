...
- `androidApp/src/main`: Android app manifest, resources, assets, native playback integration, and platform provider stores.
- `shared/src/commonMain/kotlin/org/feeluown/mobile/provider`: Shared Kotlin provider implementations and network infrastructure.
...
- `./gradlew :androidApp:assembleSmartDebug`: build per-ABI smart Android debug APKs with the on-device replacement model.
...
Write Kotlin with 4-space indentation, explicit package names under `org.feeluown.mobile`, and clear type names such as `FuoPlayerController`. Keep shared business logic in `commonMain`; add `*.android.kt` or `*.ios.kt` files only for platform-specific behavior. Swift files should use standard Swift naming (`PlayerViewModel`, `IOSNativeAudioEngine`). Provider implementations should keep transport, parsing, credentials, and domain mapping separated.
...
Recent history uses Conventional Commit-style subjects, for example `feat: implement stage two music player` and `fix: stabilize provider network cache`. Keep subjects imperative and scoped to one change. Pull requests should describe user-visible behavior, list validation commands, link related issues when available, and include screenshots or recordings for UI/player changes.

## Security & Configuration Tips

Do not commit local SDK paths, signing keys, or machine-specific paths from `local.properties`. Provider defaults belong in `androidApp/src/main/assets/providers.json`; provider implementations and network dependencies belong in the Kotlin source and Gradle version catalog.
